...
public class ProductController {
    @Autowired
    GiaodichService productService;
    @Autowired
    CategoryService categoryService;

    @GetMapping("")
    public String getList(@PageableDefault(size = 5) Pageable pageable, Model model) {
        Page<Giaodich> products = productService.getAll(pageable);
        boolean isASC = false;
...
//        model.addAttribute("times", LocalDateTime.now());
        return new ModelAndView("create", "product", new Giaodich());
    }

    @PostMapping("/create")
    public String save(@Validated @ModelAttribute Giaodich product, BindingResult bindingResult, RedirectAttributes redirect, Model model) {
    ...
    @PostMapping("/delete")
    public String delete(Giaodich product, RedirectAttributes redirect) {
    ...
    @PostMapping("/edit")
    public String update(@Validated @ModelAttribute Giaodich product, BindingResult bindingResult, RedirectAttributes redirect, Model model) {
        if (bindingResult.hasFieldErrors()) {
            model.addAttribute("categories", categoryService.findAll());
            return "edit";
